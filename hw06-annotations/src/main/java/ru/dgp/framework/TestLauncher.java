...
public final class TestLauncher {
    ...
    private static void showStatistics(Class<?> testClass, List<TestState> testStates) {
        System.out.println("Statistics for test launching for class " + testClass.getName() + ": ");

        testStates.stream().forEach(System.out::println);
    }

    private static <T> TestState launchTest(Class<T> testClass, Method test, Method[] beforeMethods, Method[] afterMethods) {
