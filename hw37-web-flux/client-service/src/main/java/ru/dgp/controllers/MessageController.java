...
public class MessageController {
    ...
    private static final Long ROOM_1408 = 1408L;
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (Objects.equals(roomId, String.valueOf(ROOM_1408))) {
            return;
        }
        ...
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId), messageToSend);

        template.convertAndSend(String.format("%s%d", TOPIC_TEMPLATE, ROOM_1408), messageToSend);
    }
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        ...
        var uri = roomId == ROOM_1408 ? "/msg" : String.format("/msg/%s", roomId);
